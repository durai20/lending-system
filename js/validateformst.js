function notNull(text, error) {
  ...
  function validateId(idValue, errorElementId) {
    // Regular expression to match only alphanumeric characters
    var alphanumericRegex = /^[a-zA-Z0-9]+$/;
  
    // Check if the ID matches the alphanumeric pattern
    if (!alphanumericRegex.test(idValue)) {
      // Display an error message
      document.getElementById(errorElementId).textContent = 'Please enter a valid alphanumeric ID.';
      return false; // Return false to indicate validation failure
    } else {
      // Clear the error message if the ID is valid
  ...
  function validatePassword(password, error) {
    ...
    if (result) {
      ...
      // Handle the case where the element with the specified error ID doesn't exist
      console.error("Element with ID " + error + " not found.");
      return false;
    }
  }
